...
async function collectMarkdown(
  root,
  current = root,
  ignoredRootDirectories = new Set(),
) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      files.push(
        ...(await collectMarkdown(root, path, ignoredRootDirectories)),
      );
...
const missingRequired = requiredPages.filter(
  (page) => !expected.includes(page),
);
if (missingRequired.length)
  failures.push(`required pages missing: [${missingRequired.join(", ")}]`);
...
console.log(
  `i18n parity verified for ${expected.length} page(s) in ${locales.join(", ")}`,
);
