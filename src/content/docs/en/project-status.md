...
| Android         | Experimental | The pin is `0.22.0`, with the `0.13.0`–`0.22.0` sync subset explicitly compatible. Startup prioritizes local recordings and does not require login or a server connection. JVM/Ktlint, Debug/Release lint, APK/AAB, and compilation of 43 instrumentation methods pass; a development-signed APK is available while physical-device execution is user acceptance. |
...
Docker or Android device checks.
