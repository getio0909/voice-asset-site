...
托管 API 35 模拟器的运行门禁已在 Android CI `29667693126` 执行 44 个 instrumentation
测试；真机运行仍未通过。未移入回收站的缓存资产还可先读取最新强 ETag，再用精确
...
- Server、Console、MCP 和 Site 的本地测试、静态检查及构建通过。Android 的 134 个
  ...
  141 组件 SBOM/校验和门禁通过；Android CI `29667693126` 实际执行 44 个 API 35
  托管模拟器 instrumentation 测试。真机运行或正式签名仍未描述为已完成。
